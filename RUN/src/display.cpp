...
void Display::lv_ex_led_1(void) {
    ...
    lv_obj_align(led1, NULL, LV_ALIGN_CENTER, 127, 80);
    lv_obj_set_size(led1, 95, 25);

    lv_obj_t * led2 = lv_led_create(adv_tab, NULL);
    lv_obj_align(led2, NULL, LV_ALIGN_CENTER, -25, -80);
...
void Display::createTitle(void){
    ...
    lv_obj_align(label1, NULL, LV_ALIGN_CENTER, 127, -35);
    lv_label_set_text(label1, "FLYWHEEL:");

    lv_obj_t*label2 = lv_label_create(adv_tab, NULL);
    lv_obj_align(label2, NULL, LV_ALIGN_CENTER, 0, -35);
    lv_label_set_text(label2, "HARDCORE ANAL MIDGET PORN:");
}
...
{
  if(master.get_digital(DIGITAL_L1) || master.get_digital(DIGITAL_L2)){lv_led_on(led1);}
